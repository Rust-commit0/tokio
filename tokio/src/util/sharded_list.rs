...
impl<L: ShardedListItem> ShardedList<L, L::Target> {
    ...
    fn shard_inner(
        &self,
        id: usize,
    ) -> MutexGuard<'_, LinkedList<L, <L as Link>::Target>> {
        panic!("STUB: not implemented");
    }
}
...
cfg_taskdump! {
    impl < L : ShardedListItem > ShardedList < L, L::Target > { pub (crate) fn for_each <
    F > (& self, mut f : F) where F : FnMut(& L::Handle), { let mut guards =
