...
cfg_windows! {
    ...
    AsSocket, BorrowedSocket };
}
cfg_net! {
    #[doc = " A TCP socket that has not yet been converted to a `TcpStream` or"] #[doc =
    " `TcpListener`."] #[doc = ""] #[doc =
    " `TcpSocket` wraps an operating system socket and enables the caller to"] #[doc =
    " configure the socket before establishing a TCP connection or accepting"] #[doc =
    ...
    " The underlying socket is closed when the `TcpSocket` value is dropped."] #[doc =
    ""] #[doc =
    " `TcpSocket` should only be used directly if the default configuration used"] #[doc
    = " by `TcpStream::connect` and `TcpListener::bind` does not meet the required"]
    #[doc = " use case."] #[doc = ""] #[doc =
    " Calling `TcpStream::connect(\"127.0.0.1:8080\")` is equivalent to:"] #[doc = ""]
    #[doc = " ```no_run"] #[doc = " use tokio::net::TcpSocket;"] #[doc = ""] #[doc =
    " use std::io;"] #[doc = ""] #[doc = " #[tokio::main]"] #[doc =
    " async fn main() -> io::Result<()> {"] #[doc =
    "     let addr = \"127.0.0.1:8080\".parse().unwrap();"] #[doc = ""] #[doc =
    "     let socket = TcpSocket::new_v4()?;"] #[doc =
    "     let stream = socket.connect(addr).await?;"] #[doc = " # drop(stream);"] #[doc =
    ""] #[doc = "     Ok(())"] #[doc = " }"] #[doc = " ```"] #[doc = ""] #[doc =
    " Calling `TcpListener::bind(\"127.0.0.1:8080\")` is equivalent to:"] #[doc = ""]
    #[doc = " ```no_run"] #[doc = " use tokio::net::TcpSocket;"] #[doc = ""] #[doc =
    " use std::io;"] #[doc = ""] #[doc = " #[tokio::main]"] #[doc =
    " async fn main() -> io::Result<()> {"] #[doc =
    "     let addr = \"127.0.0.1:8080\".parse().unwrap();"] #[doc = ""] #[doc =
    "     let socket = TcpSocket::new_v4()?;"] #[doc =
    "     // On platforms with Berkeley-derived sockets, this allows to quickly"] #[doc =
    "     // rebind a socket, without needing to wait for the OS to clean up the"] #[doc
    = "     // previous one."] #[doc = "     //"] #[doc =
    "     // On Windows, this allows rebinding sockets which are actively in use,"] #[doc
    = "     // which allows \"socket hijacking\", so we explicitly don't set it here."]
    #[doc =
    "     // https://docs.microsoft.com/en-us/windows/win32/winsock/using-so-reuseaddr-and-so-exclusiveaddruse"]
    #[doc = "     socket.set_reuseaddr(true)?;"] #[doc = "     socket.bind(addr)?;"]
    #[doc = ""] #[doc =
    ...
    #[doc = "     // illustrative and does not reflect the real value used."] #[doc =
    "     let listener = socket.listen(1024)?;"] #[doc = " # drop(listener);"] #[doc =
    ""] #[doc = "     Ok(())"] #[doc = " }"] #[doc = " ```"] #[doc = ""] #[doc =
    " Setting socket options not explicitly provided by `TcpSocket` may be done by"]
    #[doc = " accessing the `RawFd`/`RawSocket` using [`AsRawFd`]/[`AsRawSocket`] and"]
    #[doc = " setting the option with a crate like [`socket2`]."] #[doc = ""] #[doc =
    " [`RawFd`]: https://doc.rust-lang.org/std/os/fd/type.RawFd.html"] #[doc =
    " [`RawSocket`]: https://doc.rust-lang.org/std/os/windows/io/type.RawSocket.html"]
    #[doc = " [`AsRawFd`]: https://doc.rust-lang.org/std/os/fd/trait.AsRawFd.html"] #[doc
    =
    " [`AsRawSocket`]: https://doc.rust-lang.org/std/os/windows/io/trait.AsRawSocket.html"]
    #[doc = " [`socket2`]: https://docs.rs/socket2/"] #[cfg_attr(docsrs, doc(alias =
    "connect_std"))] pub struct TcpSocket { inner : socket2::Socket, }
}
impl TcpSocket {
...
cfg_unix_or_wasi! {
    impl AsRawFd for TcpSocket { fn as_raw_fd(& self) -> RawFd { self.inner.as_raw_fd() }
    } impl AsFd for TcpSocket { fn as_fd(& self) -> BorrowedFd <'_ > { unsafe {
    BorrowedFd::borrow_raw(self.as_raw_fd()) } } } impl FromRawFd for TcpSocket { #[doc =
    " Converts a `RawFd` to a `TcpSocket`."] #[doc = ""] #[doc = " # Notes"] #[doc = ""]
    #[doc = " The caller is responsible for ensuring that the socket is in"] #[doc =
    " non-blocking mode."] unsafe fn from_raw_fd(fd : RawFd) -> TcpSocket { let inner =
    unsafe { socket2::Socket::from_raw_fd(fd) }; TcpSocket { inner } } } impl IntoRawFd
    for TcpSocket { fn into_raw_fd(self) -> RawFd { self.inner.into_raw_fd() } }
}
cfg_windows! {
    impl IntoRawSocket for TcpSocket { fn into_raw_socket(self) -> RawSocket { self.inner
    .into_raw_socket() } } impl AsRawSocket for TcpSocket { fn as_raw_socket(& self) ->
    RawSocket { self.inner.as_raw_socket() } } impl AsSocket for TcpSocket { fn
    as_socket(& self) -> BorrowedSocket <'_ > { unsafe { BorrowedSocket::borrow_raw(self
    .as_raw_socket()) } } } impl FromRawSocket for TcpSocket { #[doc =
    " Converts a `RawSocket` to a `TcpStream`."] #[doc = ""] #[doc = " # Notes"] #[doc =
    ""] #[doc = " The caller is responsible for ensuring that the socket is in"] #[doc =
    " non-blocking mode."] unsafe fn from_raw_socket(socket : RawSocket) -> TcpSocket {
    let inner = unsafe { socket2::Socket::from_raw_socket(socket) }; TcpSocket { inner }
    } }
}
